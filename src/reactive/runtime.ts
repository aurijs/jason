...
export function handle_error(error, effect, previous_effect, component_context) {
    ...
    while (current_context !== null) {
        if (DEV) {
            /** @type {string} */
            const filename = current_context.function?.[FILENAME];
...
function schedule_possible_effect_self_invalidation(signal, effect, depth = 0) {
    ...
    for (const i = 0; i < reactions.length; i++) {
...
export function update_reaction(reaction) {
    ...
    const previous_untracked_writes = untracked_writes;
    const previous_reaction = active_reaction;
    const previous_skip_reaction = skip_reaction;
    const prev_derived_sources = derived_sources;
    const previous_component_context = component_context;
    const flags = reaction.f;
    ...
    try {
        const result = /** @type {Function} */ (0, reaction.fn)();
...
function remove_reaction(signal, dependency) {
    ...
    if (reactions !== null) {
        const index = index_of.call(reactions, signal);
        if (index !== -1) {
...
export function remove_reactions(signal, start_index) {
    ...
    for (const i = start_index; i < dependencies.length; i++) {
...
export function update_effect(effect) {
    ...
    if (DEV) {
        const previous_component_fn = dev_current_component_function;
    ...
    if (flush_count > 1000) {
        ...
        try {
            ...
            if (lastScheduledEffect !== null) {
                if (DEV) {
                    try {
                        handle_error(error, lastScheduledEffect, null, null);
                    } catch (e) {
...
function process_effects(effect, collected_effects) {
    const current_effect = effect.first;
    const effects = [];

    main_loop: while (current_effect !== null) {
        const flags = current_effect.f;
        const is_branch = (flags & BRANCH_EFFECT) !== 0;
        const is_skippable_branch = is_branch && (flags & CLEAN) !== 0;
        const sibling = current_effect.next;
        ...
        if (!is_skippable_branch && (flags & INERT) === 0) {
            if ((flags & RENDER_EFFECT) !== 0) {
                ...
                const child = current_effect.first;
        ...
        if (sibling === null) {
            ...
            while (parent !== null) {
                ...
                const parent_sibling = parent.next;
...
export function get(signal) {
    ...
    if (is_derived && (flags & DESTROYED) !== 0) {
        const value = execute_derived(/** @type {Derived} */(signal));
    ...
    if (active_reaction !== null) {
        ...
        const derived = /** @type {Derived} */ (signal);
        ...
        while (parent !== null) {
            ...
            if ((parent.f & DERIVED) !== 0) {
                const parent_derived = /** @type {Derived} */ (parent);

                target = parent_derived;
                parent = parent_derived.parent;
            } else {
                const parent_effect = /** @type {Effect} */ (parent);
    ...
    if (
        ...
        if (signal.debug) {
            ...
            const entry = tracing_expressions.entries.get(signal);
...
/**
 * Like `get`, but checks for `undefined`. Used for `const` declarations because they can be accessed before being declared
 * @template V
...
export function capture_signals(fn) {
    ...
    const captured = captured_signals;
    const signal;
...
export function invalidate_inner_signals(fn) {
    const captured = capture_signals(() => untrack(fn));
...
export function pop(component) {
    ...
    if (context_stack_item !== null) {
        ...
        if (component_effects !== null) {
            ...
            try {
                for (const i = 0; i < component_effects.length; i++) {
                    const component_effect = component_effects[i];
...
export function deepRead(value: unknown, visited = new Set<unknown>()) {
    if (
        typeof value === 'object' &&
        value !== null &&
